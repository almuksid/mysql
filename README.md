...
# 2. Task 2 all-in-one business database feel 
## 01. Create Database
```cmd
mysql --u root -p [open mysql terminal]
```
```sql
...
CREATE TABLE `products`(
    ...
    FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (`catagory_id`) REFERENCES `catagories`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE `invoices` (
    ...
    FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (`customar_id`) REFERENCES `customers`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
